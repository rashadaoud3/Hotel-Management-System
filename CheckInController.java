...
public class CheckInController implements Initializable {
    ...
    private void insertRoomNo() {
        ...
        String query = "SELECT RoomNumber FROM room WHERE Type=? AND Status='Not Booked'";
        try {
            pst = connection.prepareStatement(query);
    ...
    public void handleSelectRoomNumber(javafx.event.ActionEvent actionEvent) {
        ...
        String priceQuery = "SELECT Ppice FROM room WHERE RoomNumber=?";
        try {
            pst = connection.prepareStatement(priceQuery);
            pst.setString(1, no);
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                priceVal = priceVal + rs.getString("Ppice");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void handleSubmitAction(javafx.event.ActionEvent actionEvent) {
        ...
        if (name.equals("") || email.equals("") || gender.equals("") || nationality.equals("")
            ...
            if (isGuestIDExists(number)) {
                String suggestedNumber = getSuggestedGuestID();
                OptionPane("Guest ID already exists. Please enter a new one.\nSuggested guest ID: " + suggestedNumber, "Error Message");
            } else {
                String insertCustomer = "INSERT INTO guest(GuestID, RoomNumber, GuestName, Nationaility, Gender, Phone, Email)"
                        + "VALUES (?, ?, ?, ?, ?, ?, ?)";
                String insertReservation = "INSERT INTO booking(GuestID, RoomNumber, CheckinDate, CheckoutDate) VALUES (?, ?, ?, ?)";
                String updateRoom = "UPDATE room SET Status=\"Booked\" WHERE RoomNumber=?";
                try {
    ...
    private boolean isGuestIDExists(String guestID) {
        String query = "SELECT GuestID FROM guest WHERE GuestID = ?";
        try {
            pst = connection.prepareStatement(query);
    ...
    private String getSuggestedGuestID() {
        int suggestedID = 1;
        while (isGuestIDExists(String.valueOf(suggestedID))) {
            suggestedID++;
        }
